...
public sealed class DocumentPdfBuilderTests
{
    ...
    [Theory]
    [InlineData("load", 5, DocumentExportState.Queued, ProcessingJobStatus.Queued)]
    [InlineData("load", 6, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    [InlineData("begin", 6, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    [InlineData("save", 5, DocumentExportState.Queued, ProcessingJobStatus.Queued)]
    [InlineData("save", 6, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    [InlineData("commit", 6, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    [InlineData("committed", 6, DocumentExportState.Ready, ProcessingJobStatus.Failed)]
    [InlineData("load", 7, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    [InlineData("processing", 6, DocumentExportState.Failed, ProcessingJobStatus.Failed)]
    public async Task Worker_InfrastructureFailure_ReconcilesExportBeforeExhaustingJob(
        string failureStage, int attempt, DocumentExportState expectedExport, ProcessingJobStatus expectedJob)
    {
        await using var f = await Fixture.CreateAsync();
        var job = ProcessingJob.Create(Guid.NewGuid(), "BuildDocumentPdf", f.Export.Id.ToString(), "export-test", f.Clock.UtcNow);
        if (failureStage == "processing") f.Export.Start(f.Clock.UtcNow);
        for (var i = 0; i < attempt; i++) job.Lease("worker", f.Clock.UtcNow.AddMinutes(i * 3), TimeSpan.FromMinutes(2));
        f.Db.Add(job);
        await f.Db.SaveChangesAsync();
        var failure = new DatabaseFailure(failureStage == "processing" ? "load" : failureStage);
        var services = new ServiceCollection();
        services.AddScoped(_ => new AppDbContext(new DbContextOptionsBuilder<AppDbContext>()
            .UseSqlite(f.Connection).AddInterceptors(new FailExportQuery(failure), new FailExportSave(failure), new FailExportTransaction(failure)).Options));
        services.AddSingleton<IObjectStore>(f.Store);
        services.AddSingleton<IClock>(f.Clock);
        services.AddScoped<DocumentPdfBuilder>();
        services.AddScoped<IProcessingJobQueue>(sp => new LeasedQueue(new PostgresJobQueue(sp.GetRequiredService<AppDbContext>(), f.Clock),
            new(job.Id, job.Type, job.Payload, attempt, job.LeaseExpiresAt!.Value)));
        await using var provider = services.BuildServiceProvider();
        var runner = new UploadValidationJobRunner(provider.GetRequiredService<IServiceScopeFactory>(), NullLogger<UploadValidationJobRunner>.Instance);

        Assert.True(await runner.RunOnceAsync("worker", TimeSpan.FromMinutes(2), default));

        Assert.True(failure.Fired);
        var export = await f.ReloadAsync();
        Assert.Equal(expectedExport, export.State);
        Assert.Equal(expectedExport == DocumentExportState.Failed ? "export_build_failed" : null, export.FailureCode);
        Assert.Equal(expectedExport == DocumentExportState.Ready, export.OutputObjectKey is not null);
        var savedJob = await f.Db.ProcessingJobs.SingleAsync();
        Assert.Equal(expectedJob, savedJob.Status);
        Assert.Equal("export_build_failed", savedJob.ErrorCode);
    }

    [Theory]
    [InlineData(3, false, DocumentExportState.Ready)]
    [InlineData(2, false, DocumentExportState.Failed)]
    [InlineData(3, true, DocumentExportState.Failed)]
    public async Task PublicationConflict_ValidatesExistingPdfAndNeverReplacesIt(int pageCount, bool corrupt, DocumentExportState expected)
    {
        await using var f = await Fixture.CreateAsync();
        using var pdf = new PdfDocument();
        for (var i = 0; i < pageCount; i++) pdf.AddPage();
        using var output = new MemoryStream();
        pdf.Save(output, false);
        var published = corrupt ? "broken PDF"u8.ToArray() : output.ToArray();
        var key = $"exports/{f.Document.Id}/{f.Export.Id}/document.pdf";
        // The object appears after the builder's initial HEAD but before publication.
        f.Store.BeforeCreate = objectKey => f.Store.Objects.Add(objectKey, published);

        await f.BuildAsync();

        var export = await f.ReloadAsync();
        Assert.Equal(published, f.Store.Objects[key]);
        Assert.Contains(key, f.Store.ReadKeys);
        Assert.Equal(expected, export.State);
        Assert.Equal(expected == DocumentExportState.Failed ? "export_build_failed" : null, export.FailureCode);
        Assert.Equal(expected == DocumentExportState.Ready ? key : null, export.OutputObjectKey);
    }

    [Fact]
    public async Task CanceledWriter_AfterLockRelease_CannotReplaceReadyBytesWhenRemotePutFinishesLate()
    {
        await using var f = await Fixture.CreateAsync();
        using var leaseLost = new CancellationTokenSource();
        var started = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var release = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        Task<ObjectCreationResult>? remoteWrite = null;
        byte[]? staleBytes = null;
        f.Store.DelayedCreate = async (key, bytes, immutable, ct) =>
        {
            staleBytes = bytes;
            remoteWrite = CommitLateAsync();
            started.SetResult();
            return await remoteWrite.WaitAsync(ct);

            async Task<ObjectCreationResult> CommitLateAsync()
            {
                await release.Task;
                if (immutable && !f.Store.Objects.TryAdd(key, bytes)) return ObjectCreationResult.AlreadyExists;
                f.Store.Objects[key] = bytes;
                return ObjectCreationResult.Created;
            }
        };
        var first = new DocumentPdfBuilder(f.Db, f.Store, f.Clock).BuildAsync(f.Export.Id, leaseLost.Token);
        await started.Task.WaitAsync(TimeSpan.FromSeconds(10));
        try
        {
            // Cancellation releases the transaction/lock, but the remote PUT can still complete.
            leaseLost.Cancel();
            await Assert.ThrowsAnyAsync<OperationCanceledException>(() => first);
            Assert.Equal(DocumentExportState.Queued, (await f.ReloadAsync()).State);
            f.Store.DelayedCreate = null;
            await f.BuildAsync();
            var export = await f.ReloadAsync();
            Assert.Equal(DocumentExportState.Ready, export.State);
            var published = f.Store.Objects[export.OutputObjectKey!];
            Assert.NotEqual(staleBytes, published);
            release.SetResult();
            Assert.Equal(ObjectCreationResult.AlreadyExists, await remoteWrite!);
            Assert.Equal(published, f.Store.Objects[export.OutputObjectKey!]);
            Assert.Equal(DocumentExportState.Ready, (await f.ReloadAsync()).State);
        }
        finally
        {
            ...
            if (remoteWrite is not null) await remoteWrite;
        }
    }

    [Theory]
    [InlineData("BuildDocumentPdf")]
    [InlineData("ValidateUpload")]
    [InlineData("DetectDocumentEdges")]
    [InlineData("ApplyPerspectiveCrop")]
    public async Task DefaultRetryPolicy_RetriesFifthAttemptAndFailsSixth(string type)
    {
        await using var f = await Fixture.CreateAsync();
        var job = ProcessingJob.Create(Guid.NewGuid(), type, Guid.NewGuid().ToString(), "retry-test", f.Clock.UtcNow);
        for (var i = 0; i < 5; i++) job.Lease("worker", f.Clock.UtcNow.AddMinutes(i * 3), TimeSpan.FromMinutes(2));
        f.Db.Add(job);
        await f.Db.SaveChangesAsync();
        var queue = new PostgresJobQueue(f.Db, f.Clock);
        await queue.RescheduleAsync(job.Id, "worker", "safe_error", default);
        Assert.Equal(ProcessingJobStatus.Queued, job.Status);
        Assert.Equal(f.Clock.UtcNow.AddMinutes(30), job.AvailableAt);
        job.Lease("worker", job.AvailableAt, TimeSpan.FromMinutes(2));
        await f.Db.SaveChangesAsync();
        await queue.RescheduleAsync(job.Id, "worker", "safe_error", default);
        f.Db.ChangeTracker.Clear();
        Assert.Equal(ProcessingJobStatus.Failed, (await f.Db.ProcessingJobs.SingleAsync()).Status);
    }
    ...
    private sealed class Store : IObjectStore
    {
        ...
        public Func<string, byte[], bool, CancellationToken, Task<ObjectCreationResult>>? DelayedCreate { get; set; }
        public Task<ObjectCreationResult> WriteIfAbsentAsync(string key, string mediaType, Stream content, CancellationToken ct) =>
            WriteCoreAsync(key, mediaType, content, true, ct);
        private async Task<ObjectCreationResult> WriteCoreAsync(string key, string mediaType, Stream content, bool immutable, CancellationToken ct)
        {
            Assert.Equal("application/pdf", mediaType);
            using var copy = new MemoryStream();
            await content.CopyToAsync(copy, ct);
            BeforeCreate?.Invoke(key);
            if (DelayedCreate is { } delayed) return await delayed(key, copy.ToArray(), immutable, ct);
            if (immutable && !Objects.TryAdd(key, copy.ToArray())) return ObjectCreationResult.AlreadyExists;
            if (!immutable) Objects[key] = copy.ToArray();
            Writes++;
            OnWrite?.Invoke();
        ...
        {
            await WriteCoreAsync(key, mediaType, content, false, ct);
        }
    ...
    private sealed class DatabaseFailure(string stage)
    {
        public bool Fired { get; private set; }
        public void Check(string current)
        {
            if (Fired || stage != current) return;
            Fired = true;
            throw new IOException("Injected database/session failure containing private details");
        }
    }
    private sealed class FailExportQuery(DatabaseFailure failure) : DbCommandInterceptor
    {
        public override ValueTask<InterceptionResult<DbDataReader>> ReaderExecutingAsync(DbCommand command,
            CommandEventData eventData, InterceptionResult<DbDataReader> result, CancellationToken cancellationToken = default)
        {
            if (command.CommandText.Contains("FROM \"document_exports\"", StringComparison.Ordinal)) failure.Check("load");
            return ValueTask.FromResult(result);
        }
    }
    private sealed class FailExportSave(DatabaseFailure failure) : SaveChangesInterceptor
    {
        public override ValueTask<InterceptionResult<int>> SavingChangesAsync(DbContextEventData eventData,
            InterceptionResult<int> result, CancellationToken cancellationToken = default)
        {
            if (eventData.Context!.ChangeTracker.Entries<DocumentExport>().Any(x => x.Entity.State == DocumentExportState.Ready)) failure.Check("save");
            return ValueTask.FromResult(result);
        }
    }
    private sealed class FailExportTransaction(DatabaseFailure failure) : DbTransactionInterceptor
    {
        ...
        { failure.Check("begin"); return ValueTask.FromResult(result); }
        public override ValueTask<InterceptionResult> TransactionCommittingAsync(DbTransaction transaction,
            TransactionEventData eventData, InterceptionResult result, CancellationToken cancellationToken = default)
        { failure.Check("commit"); return ValueTask.FromResult(result); }
        public override Task TransactionCommittedAsync(DbTransaction transaction, TransactionEndEventData eventData,
            CancellationToken cancellationToken = default)
        { failure.Check("committed"); return Task.CompletedTask; }
    }
    private sealed class LeasedQueue(IProcessingJobQueue inner, ProcessingJobLease lease) : IProcessingJobQueue
    {
        public Task<ProcessingJobLease?> TryLeaseAsync(string worker, TimeSpan duration, CancellationToken ct) => Task.FromResult<ProcessingJobLease?>(lease);
        public Task CompleteAsync(Guid id, string worker, CancellationToken ct) => inner.CompleteAsync(id, worker, ct);
        public Task RescheduleAsync(Guid id, string worker, string code, CancellationToken ct) => inner.RescheduleAsync(id, worker, code, ct);
        public Task<bool> HeartbeatAsync(Guid id, string worker, TimeSpan duration, CancellationToken ct) => Task.FromResult(true);
        public Task EnqueueAsync(string type, string payload, string key, CancellationToken ct) => throw new NotSupportedException();
    }
}
